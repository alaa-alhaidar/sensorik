...
WINDOW_SECONDS = 1.0
DISPLAY_PERIODS = 10 #DEFAULT_MEASUREMENT_F0 # Anzahl der Sinusperioden, die im Zeitplot angezeigt werden sollen (z.B. 2 Perioden bei 500 Hz -> 0.004 s)
...
class SignalAnalysisScreen(QWidget):
    ...
    def _connect_signals(self): # Hier werden die Signale der UI-Elemente mit den entsprechenden Funktionen verbunden
        ...
        self.record_button.clicked.connect(self.record_for_time)
    ...
    def _reset_plot_storage(self):
        sample_rate = self._get_sample_rate()
        window_samples = max(2, int(round(self.window_seconds * sample_rate)))
    ...
    def refresh_focusrite_devices(self):
        try:
            ...
            for idx, name, ch in self.devices:
                self.device_combo.addItem(f"{idx}: {name} ({ch} in)")
    ...
    def update_time_plots(self, signal, f0):
        ...
        display_seconds = DISPLAY_PERIODS / f0 # z.B. 10 Perioden bei 1000 Hz = 0.01 s
        display_samples = int(round(display_seconds * sample_rate)) # z.B. 480 Samples bei 48 kHz für 10 Perioden bei 1000 Hz
        display_samples = max(2, min(display_samples, num_samples)) # z.B. 480 Samples bei 48 kHz für 10 Perioden bei 1000 Hz

        self.time_axis = np.arange(display_samples, dtype=np.float32) / sample_rate

        for ch in range(self.num_channels):
            self.plot_buffers[ch] = signal[:display_samples, ch]
            self.time_plot_curves[ch].setData(self.time_axis, self.plot_buffers[ch])
    ...
    def record_for_time(self,duration=MEASUREMENT_DURATION):
        try:
            self.timer.stop()
            ...
            self.log(f"Aufnahme abgeschlossen: Dauer={MEASUREMENT_DURATION:.3f} s")
            self.log(f"Samples pro Mikrofon: {num_samples}")
            self.log(f"Messfrequenz: {f0:.2f} Hz")
            self.log("Frequenzauflösung: 1 Hz")
            self.log("-" * 100)
    ...
    def update_audio_plot(self): # live-Plot mit neuen Daten aus dem Audio-Interface aktualisieren
        ...
        while not self.focusrite.audio_queue.empty():
            ...
            if kind == "audio":
                ...
                for ch in range(self.num_channels):
                    channel_chunk = chunk[:, ch]
                    n = len(channel_chunk)
                    target_len = len(self.plot_buffers[ch])

                    if n >= target_len:
                        self.plot_buffers[ch] = channel_chunk[-target_len:]
                    else:
                        self.plot_buffers[ch][:-n] = self.plot_buffers[ch][n:]
                        self.plot_buffers[ch][-n:] = channel_chunk

                self.live_elapsed_samples += chunk.shape[0]
                end_time = self.live_elapsed_samples / self._get_sample_rate()

                if end_time < self.window_seconds:
                    axis_start = 0.0
                    axis_end = self.window_seconds
                else:
                    axis_start = end_time - self.window_seconds
                    axis_end = end_time

                self.time_axis = np.linspace(
                    axis_start,
                    axis_end,
                    len(self.plot_buffers[0]),
                    endpoint=False,
                    dtype=np.float32,
                )
                updated = True
